...
def anagram?(x, y)
  (x.split('') - y.split('')).empty? && x.size == y.size
end

def has_anagram?(array)
  i = 0
  flag = false
  while i < array.size do
    j = 0
    while j < array.size do
      if i != j && anagram?(array[i], array[j])
        flag = true
      end
      j += 1
    end
...
answer1 = input.reduce(0) {|acc,x| acc + ((!has_duplicates?(x)) ? 1 : 0)}
answer2 = input.reduce(0) {|acc,x| acc + ((!has_anagram?(x) && !has_duplicates?(x)) ? 1 : 0)}
